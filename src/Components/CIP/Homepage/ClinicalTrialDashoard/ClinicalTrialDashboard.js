...
import ClinicalTrialDashboard from './ClinicalTrialDashboard';

const ClinicalTrialDashboardByFunctionalComponent = () => {

   const [info,setInfo] = useState({});
   const [developmentPhaseArray,setDevelopmentPhaseArray] = useState({});

    useEffect( () => {
        axios.get(`http://172.21.192.44:7171/ctdata/studystatusinfo`)
        .then(response => { setInfo(response.data)})
        // .then(response.send(info))
},[])

useEffect( () => {
    axios.get(`http://172.21.192.44:7171/ctdata/studyphaseinfo`)
    .then(response => response.data)
    .then((data) => {setDevelopmentPhaseArray(data)})
},[])
                ...
                <div className="row margin0">
                    <div className="col-sm-4 col-xs-12 paperWrapper ">
                        <Paper >
                            ...
                            <p className="margin0">{info.data !=undefined ? info.data.totalnumber: ""  } </p>
                        </Paper>
                    </div>
                            ...
                            <div className="row margin0">
                                <div className="col-sm-3 col-xs-12">
                                    <div >
                                        ...
                                        <p className="margin0">{info.data != undefined ? info.data.completed : ""}  </p>
                                    </div>
                                </div>
                                <div className="col-sm-3 col-xs-12">
                                    <div >
                                        <p className="margin0"> ONGOING</p>
                                        <p className="margin0">{info.data != undefined ? info.data.ongoing : ""}</p>
                                    </div>
                                </div>
                                <div className="col-sm-3 col-xs-12">
                                    <div >
                                        <p className="margin0"> PLANNED</p>
                                        <p className="margin0">{info.data != undefined ? info.data.planned : ""}</p>
                                    </div>
                                </div>
                                <div className="col-sm-3 col-xs-12">
                                    <div >
                                        <p className="margin0"> TERMINATED</p>
                                        <p className="margin0">{info.data != undefined ? info.data.terminated : ""}</p>
                                    </div>
                                </div>
                            </div>
                        </Paper >
                ...
                <div className="row margin0">
                    <div className="col-sm-4 col-xs-12 cardContainer chartCardContainer paddingRight0">
                        <div className="clinicalDoughnutWrapper">
                            <CardWithChartComponent chartArray={developmentPhaseArray} title="Development Phase" type="Clinical Trial Development Phase" width="380" />
                        </div>
                    </div>
...
export default connect(null,null)(ClinicalTrialDashboardByFunctionalComponent);